...
`run_doctor` short-circuits the normal report and prints one finding per actionable issue, plus a recommendation. Checks: `MemAvailable < 10%` of `MemTotal` (includes top-5 RSS via `lightweight_top_rss`, which reads `/proc/<pid>/status`'s `VmRSS` rather than parsing smaps — much faster); unused hugetlbfs files (root-only, dedup by `(dev, inode)`, reuses `hugetlbfs_holders`); idle hugepage pools — any pool with `free > 0` recommends `--release`. When the pool has reserved pages, the recommendation includes a warning that `--release` will also drop those reservations and break the next page-fault on any still-unfaulted mapping. Each finding has `title`, optional `lines`, optional `recommendation`. The function prints "No issues found." when nothing fired and always returns 0 — exit codes are not used to signal findings (operators pipe the output, scripts grep for "No issues found").
...
`release_hugepages` computes `target = nr_hugepages - free_hugepages` per size and writes it to `/sys/kernel/mm/hugepages/hugepages-*kB/nr_hugepages`, then reads back to surface what the kernel actually freed. This is aggressive: it drops reserved-but-unfaulted pages too, so any process that had mmap'd hugetlbfs but hadn't yet faulted will SIGBUS on its next access — that's an explicit trade-off, not a bug. If readback shows the pool larger than `target`, the kernel couldn't shrink that far (pages were still in use at write time, e.g. faulted between our read and the kernel's processing of the write). When both flags are given, `--unlink` runs first so dead hugetlbfs files release their backing pages before `--release` drains the pool. `_unescape_proc` handles the kernel's `\\040` / `\\134` / `\\011` / `\\012` escapes that appear in `/proc/mounts` and `/proc/<pid>/maps` paths.
