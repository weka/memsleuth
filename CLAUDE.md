...
`run_doctor` short-circuits the normal report and prints one finding per actionable issue, plus a recommendation. Checks: `MemAvailable < 10%` of `MemTotal` (includes top-5 RSS via `lightweight_top_rss`, which reads `/proc/<pid>/status`'s `VmRSS` rather than parsing smaps — much faster); unused hugetlbfs files (root-only, dedup by `(dev, inode)`, reuses `hugetlbfs_holders`); idle hugepage pools — any pool with unreserved free pages (`free - resv > 0`) recommends `--release` (reserved pages are kept, the kernel won't shrink below reserved + in-use); surplus hugepages — any pool with `surplus > 0` (usually the aftermath of someone shrinking `nr_hugepages` while pages were in use) recommends `--release`, which absorbs the surplus back into the persistent pool. The hugepage check fires when `releasable > 0` **or** `surplus > 0`, and the finding's wording adapts to which condition(s) hold. Each finding has `title`, optional `lines`, optional `recommendation`. The function prints "No issues found." when nothing fired and always returns 0 — exit codes are not used to signal findings (operators pipe the output, scripts grep for "No issues found").
...
`release_hugepages` shrinks each pool to `reserved + in-use` and reads back to surface what the kernel actually freed. **Critical hugepage-accounting fact:** `nr_hugepages` reads back as the kernel's `h->nr_huge_pages`, which is the **total** pool — it *already includes surplus pages* (`persistent = nr - surplus`). A write to `nr_hugepages`, however, is interpreted as the **persistent-pool** target. Two consequences the code must honor (both were real bugs):

- **Surplus.** When surplus > 0, a plain `nr - free` write is `>= persistent`, so the kernel merely converts surplus pages to persistent and frees nothing (`--release` appeared to do nothing). Fix: absorb surplus first by writing the current total (promotes surplus→persistent without allocating), then re-read and shrink. This mirrors the manual two-step operators discovered ("set nr_hugepages to its current value, then release"). `--release` absorbs surplus **whenever it is present, even if nothing is freeable** (e.g. a pool shrunk to all-surplus while fully in use) — this normalizes the pool, and is what `--doctor`'s surplus alert recommends. Side effect: absorbed in-use pages become persistent, so they no longer auto-return when their process exits (a later `--release` reclaims them).
- **Reserved.** The kernel floors any shrink at `min_count = resv + nr - free` (reserved + in-use) and never goes below it, so reserved pages **cannot** be dropped through `nr_hugepages`. The target is therefore `_release_target(nr, free, resv) = nr - max(0, free - resv)`, which equals that floor so readback matches the target cleanly. (An earlier version wrote `nr - free` claiming to "drop reserved too" — physically impossible; it just produced a misleading "kept N in use" message.)
...
1. **Persistent pool** (`numa_hugetlb` from `collect_numa_hugepages`) — `free` and `nr` per size per node (`nr` already includes surplus).
